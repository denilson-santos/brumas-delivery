$(function () {
    ...
    // Add row
    var count = $('.selected-week-days input').length ?? 0;
    var editing = false;
        ...
        currentRowElements.each( function (index, element) {           
            if (index === 0) {
                ...
                if ($(`.selected-week-days input[data-row="${currentRow.data('row')}"]`).data('row')) {
                    if ($(`.selected-week-days input[data-row="${currentRow.data('row')}"]`).hasClass('saved')) {
                        $(`.selected-week-days input[data-row="${currentRow.data('row')}"]`).removeClass('saved');
                    }
                    ...
                    $('.selected-week-days').append(`<input type="hidden" data-day-index="${currentRowElements.eq(index).data().selectric.state.selectedIdx}" data-week-day="${currentRowElements.eq(index).val()}" data-row="${currentRow.data('row')}" data-id-operation="">`);
                }
    ...
    $('.table-operation').on('click', '.edit', function () {
        ...
        // console.log(currentRowElements.eq(0).html());
    ...
    $('.table-operation').on('click', '.delete', function () {
        ...
        $(`.selected-week-days input[data-row="${rowElement.data('row')}"]`).appendTo('.deleted-week-days');
